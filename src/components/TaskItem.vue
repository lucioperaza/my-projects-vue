...
import { TASK_STATUSES, TASK_STATUSES_LABELS } from '@/lib/constants'
...
<template>
  ...
  >
    ...
    <div class="flex shrink-0 items-center gap-2">
      <span :class="chipClass[task.status]">{{ TASK_STATUSES_LABELS[task.status] }}</span>
      <label
        class="sr-only"
        :for="`status-${task.id}`"
        >Status</label
